...
class Redux_ThemeCheck {
	...
	public function __construct() {
		...
		if ( isset( $_GET['page'] ) && 'themecheck' == $_GET['page'] ) {
	...
	public function load() {
		...
		//if ( ! empty( $redux ) ) {
			...
			if ( isset( $_POST['themename'] ) && ! empty( $_POST['themename'] ) ) {
				...
				if ( false !== strpos( $dir, $_POST['themename'] ) ) : ?>
					<div class="updated">
						<p><?php echo sprintf( __( 'The theme you are testing has %s embedded. We invite you to read the %sTheme-Check Documentation%s to understand some warnings you will see because of Redux.', 'redux-framework' ), '<a href="http://reduxframework.com" target="_blank">Redux Framework</a>', '<a href="http://docs.reduxframework.com/core/theme-check/">', '</a>' ); ?></p>
					</div>
					<?php
				endif;
			...
			if ( ! ReduxFramework::$_is_plugin ) {
				...
				if ( ! empty( $errors ) ) : ?>
					<div class="error">
						<p>
							<?php _e( 'The following directories & files are still located in your <strong>Redux</strong>  directory. They may cause errors in Theme-Check.', 'redux-framework' ); ?>
