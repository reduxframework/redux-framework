...
class TIVWP_DM_Controller {
	...
	private static function _setup_automatic_switch() {
		...
		if ( ! is_callable( array(
			...
			TIVWP_DM_Notices::add( sprintf( __( 'Unknown action "%1$s" specified for %2$s', 'tivwp-dm' ),
					$action, 'TIVWP_DM_AUTO' ),
				TIVWP_DM_Notices::WITH_TRIGGER_ERROR
			);

			$action = TIVWP_DM::ACTION_DEACTIVATE;

			TIVWP_DM_Notices::add( sprintf( __( 'The unknown action has been replaced with "%s"', 'tivwp-dm' ), $action ),
				TIVWP_DM_Notices::WITH_TRIGGER_ERROR );
		 ...
		 */
		TIVWP_DM_Notices::add( sprintf( __( 'Automatic action performed: "%s"', 'tivwp-dm' ), $action,
			TIVWP_DM_Notices::WITHOUT_TRIGGER_ERROR ) );
