...
//                $comparison = $field['required'][1];
        ...
        /**
         * Pass configuration to javascript object in fields which uses it
         *
         * @since       3.1.5.11
         * @access      private
         * @return      void
         */
        private function _localize_fields($field, $is_group = false, $v = ""){
            ...
            if( isset( $field['type'] ) && $field['type'] != 'callback' ) {
                ...
                if( $class_file ) {
                    ...
                    if ( ( method_exists( $field_class, 'enqueue' ) ) || method_exists( $field_class, 'localize' ) ) {
                        ...
                        if($is_group){
                            $theField = new $field_class( $field, $v, $this );
                        }else{
                        ...
                        if($field["type"] == "group"){
                            if(is_array($this->options[$field['id']])){
                                foreach($this->options[$field['id']] as $subindex => $val){
                                    foreach($field["subfields"] as $subfield){
                                        $subfield["id"] =  $subfield["id"] . "-".$subindex;
                                        $this->_localize_fields($subfield, true, $this->options[$field['id']][$subindex][$subfield["id"]]);
                                    }
                                }
                            }
                        }
