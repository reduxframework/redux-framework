...
(function( $ ) {
    ...
    redux.field_objects.typography.select = function( selector ) {
        ...
        var family = selector.find( '#' + mainID + ' #' + mainID + '-family' ).val();

        if ( !family ) {
            family = null; //"inherit";
        }

        var familyBackup = selector.find( '#' + mainID + ' select.redux-typography-family-backup' ).val();
        var size = selector.find( '#' + mainID + ' .redux-typography-size' ).val();
        var height = selector.find( '#' + mainID + ' .redux-typography-height' ).val();
        var word = selector.find( '#' + mainID + ' .redux-typography-word' ).val();
        var letter = selector.find( '#' + mainID + ' .redux-typography-letter' ).val();
        var align = selector.find( '#' + mainID + ' select.redux-typography-align' ).val();
        var transform = selector.find( '#' + mainID + ' select.redux-typography-transform' ).val();
        var fontVariant = selector.find( '#' + mainID + ' select.redux-typography-font-variant' ).val();
        var decoration = selector.find( '#' + mainID + ' select.redux-typography-decoration' ).val();
        var style = selector.find( '#' + mainID + ' select.redux-typography-style' ).val();
        var script = selector.find( '#' + mainID + ' select.redux-typography-subsets' ).val();
        var color = selector.find( '#' + mainID + ' .redux-typography-color' ).val();
        var units = selector.find( '#' + mainID ).data( 'units' );
        ...
        if ( isSelecting === true ) {
            ...
            selector.find( '#' + mainID + ' .redux-typography-google-font' ).val( google );
        } else {
            google = redux.field_objects.typography.makeBool( selector.find( '#' + mainID + ' .redux-typography-google-font' ).val() ); // Check if font is a google font
        }

        // Page load. Speeds things up memory wise to offload to client
        if ( !selector.find( '#' + mainID ).hasClass( 'typography-initialized' ) ) {
            style = selector.find( '#' + mainID + ' select.redux-typography-style' ).data( 'value' );
            script = selector.find( '#' + mainID + ' select.redux-typography-subsets' ).data( 'value' );
        ...
        if ( $( selector ).hasClass( 'redux-typography-family' ) ) {
            ...
            if ( google === true ) {
                ...
                // destroy select2
                selector.find( '#' + mainID + ' .redux-typography-style' ).select2( "destroy" );

                // Instert new HTML
                selector.find( '#' + mainID + ' .redux-typography-style' ).html( html );

                // Init select2
                selector.find( '#' + mainID + ' .redux-typography-style' ).select2( default_params );
                ...
                // Destroy select2
                selector.find( '#' + mainID + ' .redux-typography-subsets' ).select2( "destroy" );

                // Inset new HTML
                selector.find( '#' + mainID + ' .redux-typography-subsets' ).html( html );

                // Init select2
                selector.find( '#' + mainID + ' .redux-typography-subsets' ).select2( default_params );

                selector.find( '#' + mainID + ' .redux-typography-subsets' ).parent().fadeIn( 'fast' );
                selector.find( '#' + mainID + ' .typography-family-backup' ).fadeIn( 'fast' );
            } else {
                if ( details ) {
                    $.each(
                        details, function( index, value ) {
                            if ( index === style || index === "normal" ) {
                                selected = ' selected="selected"';
                                selector.find( '#' + mainID + ' .typography-style .select2-chosen' ).text( value );
                            } else {
                                selected = "";
                            }
                    ...
                    // Prettify things
                    selector.find( '#' + mainID + ' .redux-typography-subsets' ).parent().fadeOut( 'fast' );
                    selector.find( '#' + mainID + ' .typography-family-backup' ).fadeOut( 'fast' );
                }
            }

            selector.find( '#' + mainID + ' .redux-typography-font-family' ).val( output );
        ...
        if ( selector.find( '#' + mainID + " select.redux-typography-style option[value='" + style + "']" ).length === 0 ) {
            style = "";
            selector.find( '#' + mainID + ' select.redux-typography-style' ).select2( 'val', '' );
        } else if ( style === "400" ) {
            selector.find( '#' + mainID + ' select.redux-typography-style' ).select2( 'val', style );
        }

        // Handle empty subset select
        if ( selector.find( '#' + mainID + " select.redux-typography-subsets option[value='" + script + "']" ).length === 0 ) {
            script = "";
            selector.find( '#' + mainID + ' select.redux-typography-subsets' ).select2( 'val', '' );
        }
        ...
        $( '.' + _linkclass ).remove();
        if ( family !== null && family !== "inherit" && selector.find( '#' + mainID ).hasClass( 'typography-initialized' ) ) {
            ...
            if ( google === true ) {
                ...
                selector.find( '#' + mainID + ' .redux-typography-google' ).val( true );
            } else {
                selector.find( '#' + mainID + ' .redux-typography-google' ).val( false );
            }
        }

        // Weight and italic
        if ( style.indexOf( "italic" ) !== -1 ) {
            selector.find( '#' + mainID + ' .typography-preview' ).css( 'font-style', 'italic' );
            selector.find( '#' + mainID + ' .typography-font-style' ).val( 'italic' );
            style = style.replace( 'italic', '' );
        } else {
            selector.find( '#' + mainID + ' .typography-preview' ).css( 'font-style', "normal" );
            selector.find( '#' + mainID + ' .typography-font-style' ).val( '' );
        }

        selector.find( '#' + mainID + ' .typography-font-weight' ).val( style );
        ...
        if ( size === '' ) {
            selector.find( '#' + mainID + ' .typography-font-size' ).val( '' );
        } else {
            selector.find( '#' + mainID + ' .typography-font-size' ).val( size + units );
        }

        if ( height === '' ) {
            selector.find( '#' + mainID + ' .typography-line-height' ).val( '' );
        } else {
            selector.find( '#' + mainID + ' .typography-line-height' ).val( height + units );
        }

        selector.find( '#' + mainID + ' .typography-word-spacing' ).val( word + units );
        selector.find( '#' + mainID + ' .typography-letter-spacing' ).val( letter + units );

        // Show more preview stuff
        if ( selector.find( '#' + mainID ).hasClass( 'typography-initialized' ) ) {
            var isPreviewSize = selector.find( '#' + mainID + ' .typography-preview' ).data( 'preview-size' );

            if ( isPreviewSize == '0' ) {
                selector.find( '#' + mainID + ' .typography-preview' ).css( 'font-size', size + units );
            }

            selector.find( '#' + mainID + ' .typography-preview' ).css( 'font-weight', style );

            //show in the preview box the font
            selector.find( '#' + mainID + ' .typography-preview' ).css( 'font-family', family + ', sans-serif' );

            if ( family === 'none' && family === '' ) {
                //if selected is not a font remove style "font-family" at preview box
                selector.find( '#' + mainID + ' .typography-preview' ).css( 'font-family', 'inherit' );
            }

            selector.find( '#' + mainID + ' .typography-preview' ).css( 'line-height', height + units );
            selector.find( '#' + mainID + ' .typography-preview' ).css( 'word-spacing', word + units );
            selector.find( '#' + mainID + ' .typography-preview' ).css( 'letter-spacing', letter + units );

            if ( color ) {
                selector.find( '#' + mainID + ' .typography-preview' ).css( 'color', color );
                selector.find( '#' + mainID + ' .typography-preview' ).css(
                    'background-color', redux.field_objects.typography.contrastColour( color )
                );
            }

            selector.find( '#' + mainID + ' .typography-style .select2-chosen' ).text( selector.find( '#' + mainID + ' .redux-typography-style option:selected' ).text() );
            selector.find( '#' + mainID + ' .typography-script .select2-chosen' ).text( selector.find( '#' + mainID + ' .redux-typography-subsets option:selected' ).text() );

            if ( align ) {
                selector.find( '#' + mainID + ' .typography-preview' ).css( 'text-align', align );
            }

            if ( transform ) {
                selector.find( '#' + mainID + ' .typography-preview' ).css( 'text-transform', transform );
            }

            if ( fontVariant ) {
                selector.find( '#' + mainID + ' .typography-preview' ).css( 'font-variant', fontVariant );
            }

            if ( decoration ) {
                selector.find( '#' + mainID + ' .typography-preview' ).css( 'text-decoration', decoration );
            }
            selector.find( '#' + mainID + ' .typography-preview' ).slideDown();
        }
        // end preview stuff

        // if not preview showing, then set preview to show
        if ( !selector.find( '#' + mainID ).hasClass( 'typography-initialized' ) ) {
            selector.find( '#' + mainID ).addClass( 'typography-initialized' );
        }
